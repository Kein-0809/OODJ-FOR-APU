package UzuzCollegeOOP.Encapsulation;

//! 外部に公開する = 「public」 内部内のみで使う(外部に公開しない) = 「private」
//! 基本的に「Getter」と「Setter」は書かないようにする．Setterは特に書かないようにすることを意識する
//!「Getter」と「Setter」を書かないようにする代わりに「acceleratorメソッド」や「brakeメソッド」のような物として想像ができるメソッドでフィールドの値を変えるて機能するメソッドの書き方を心がける
//! ↑の書き方の方がよりオブジェクト指向っぽい書き方ができる
...
public class Sample2_03_1_car {
	...
	//コンストラクタ①（引数なし）//! コンストラクタは基本的には「public」
	...
	//コンストラクタ②（引数あり）//! コンストラクタは基本的には「public」
	...
	//塗装色が正式なカラーバリエーション（「WHITE」「BLACK」「RED」）かをチェックする //! 値が正しいか判断すること「Validation check」という
	// このメソッドはこのクラス内で定義してある「paint()」内でしか使われず、外部でこのメソッド自体をわざわざ使うことを想定していないため「private」が適用されている
