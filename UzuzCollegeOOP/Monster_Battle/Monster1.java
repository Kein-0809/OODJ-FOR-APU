package UzuzCollegeOOP.Monster_Battle;
...
class Monster1 {
	...
	//! 「final」とは、「クラス」「変数」「フィールド」「メソッド」において1度しか宣言することができなくするもの
	...
	void setWaza(String nm, String dmr){
		...
		//バリデーションチェック：引数2が「X.X(Xは0～9の数字のいずれか)(小数第2まで)」形式で渡されたかをチェック
		...
		//! setWazaメソッドに使われているバリデーションチェックについて
		//! 使用する正規表現は: ^[0-9]+\.[0-9]$
		//! 「^」は前方一致．「$」は後方一致．「[0-9]」は単純に数字0〜9の一文字．「+」は連続体．「\」はエスケープシークエンスを率いて「.」を表す
		//! < OKの場合 >
		//! 1.5
		//! 2.0
		//! 123.4
	...
	int damaged(int orgDm){
		...
		//! 「変数名.add()は足し算を表す」
		//! 「変数名.divide()」は第1引数に値、2引数に小数点を切り捨てする場所、3引数に切り上げか切り下げ
		//! 2は小数点2までというRangeを決めている、RoundingMode.DOWNは小数点を切り捨て
