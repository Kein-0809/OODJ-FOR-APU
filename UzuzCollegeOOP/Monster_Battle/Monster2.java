...
class Monster2 {
	...
	Monster2(String tr, String nm){
		this();                          //! コンストラクタ1の呼び出し <- 27行目のコンストラクタ1にいって実行して、コンストラクタ1の値をまず参照する
		this.trainer    = tr   ;         //(初期化)トレーナー
		this.name       = nm   ;         //(初期化)なまえ
		//! つまりコンストラクタ2では42行目の「this()」でコンストラクタ1を参照しているため「this.trainer」と「this.name」以外はコンストラクタ1と同じ
	}
	//コンストラクタ3[引数1:トレーナー, 引数2:なまえ, 引数3:レベル]
	Monster2(String tr, String nm, int lev){
		this(tr, nm);                //! コンストラクタ2の呼び出し <- 41行目の引数が2個のコンスト2にいって実行して、そのコンスト2がコンスト1の値をまず参照し、コンスト2に戻り、コンスト3の引数「tr」と「nm」を受け取り「this.trainer」と「this.name」に入れ、コンスト3に戻ってくる
